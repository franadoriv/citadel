//! Postgres purchases repository.
//!
//! [`PgPurchasesRepository`] is the durable backend for
//! [`PurchasesRepository`](crate::repository::PurchasesRepository). Every
//! validated purchase is one `purchases` row keyed by its store-unique
//! `transaction_id` (the primary key, so a replayed receipt is a `Conflict`); a
//! subscription is a purchase carrying `subscription_expiry_unix_ms` (no separate
//! table). The user-filtered newest-first paging and the subscription
//! `active`/`expired` derivation are reused from the shared pure helpers in
//! [`crate::repository::purchases`], so this backend cannot drift from the
...
use super::{PgExecutor, db_err, get, millis_to_ts, tx_closed};
...
SELECT transaction_id, user_id, product_id, store, receipt_sha256, validated_at_unix_ms, \
 subscription_expiry_unix_ms FROM purchases WHERE transaction_id = $1";

// --- mapping helpers --------------------------------------------------------

fn parse_purchase(row: &PgRow) -> AppResult<Purchase> {
...
/// Postgres [`PurchasesRepository`].
pub struct PgPurchasesRepository {
    executor: PgExecutor,
}
...
#[async_trait]
impl PurchasesRepository for PgPurchasesRepository {
...
async fn record_conn(conn: &mut PgConnection, purchase: Purchase) -> AppResult<Purchase> {
    // A duplicate transaction id collides on the primary key; `db_err` maps the
    // unique violation (23505) to `Conflict`.
...
async fn load_all(conn: &mut PgConnection) -> AppResult<Vec<Purchase>> {
...
async fn get_conn(conn: &mut PgConnection, transaction_id: &str) -> AppResult<Option<Purchase>> {
