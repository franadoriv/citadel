...
//!
//! [`PgSessionRepository`] is the durable backend for
//! [`SessionRepository`](crate::repository::SessionRepository). It reproduces the
//! in-memory reference impl's hardening exactly:
//!
//! - `update_session` is a compare-and-set. The current row is locked
//!   (`SELECT ... FOR UPDATE`), rehydrated into a [`Session`], and the identical
//!   in-memory guards are re-applied on the domain objects: immutable facts
//!   (`user_id`, `issued_at`, `owner_node`) may not change, and a terminal stored
//!   session accepts only a byte-identical write. A stale refresh can therefore
//!   never resurrect a session that was revoked or expired since it was read.
//! - `revoke_user_sessions` is atomic and idempotent: within one transaction it
//!   locks every non-terminal session of the user, transitions each through the
...
//! record); the bearer [`SessionTokenSecret`](
//! crate::session::SessionTokenSecret) is never stored — it is not even
//! serializable — so no secret material reaches the database.
//!
//! # Record shape
//!
...
//! is the full session serialized into a `jsonb` `data` column. Flat query
//! columns (`id`, `user_id`, `token_ref`, `state_kind`) are projected out for
//! lookups and the revoke scan. The `token_ref` column is the lookup index: on a
//! bulk revoke it is cleared to `NULL` (so a revoked session is no longer
//! resolvable by its token) while `data` keeps the original reference, mirroring
//! the in-memory split between the by-id store and the by-token index.
...
const GET_SESSION_SQL: &str = "SELECT data FROM sessions WHERE id = $1";

const GET_SESSION_BY_TOKEN_REF_SQL: &str = "SELECT data FROM sessions WHERE token_ref = $1 LIMIT 1";

const INSERT_SESSION_SQL: &str = "\
INSERT INTO sessions (id, user_id, token_ref, state_kind, data) \
VALUES ($1, $2, $3, $4, $5)";

const LOCK_SESSION_SQL: &str = "SELECT data FROM sessions WHERE id = $1 FOR UPDATE";

const UPDATE_SESSION_SQL: &str = "\
UPDATE sessions SET token_ref = $2, state_kind = $3, data = $4 WHERE id = $1";

/// Lock every non-terminal (`active`) session of a user for a bulk revoke.
const LOCK_ACTIVE_SESSIONS_SQL: &str =
    "SELECT data FROM sessions WHERE user_id = $1 AND state_kind = 'active' FOR UPDATE";
...
UPDATE sessions SET token_ref = NULL, state_kind = 'revoked', data = $2 WHERE id = $1";

// --- mapping helpers --------------------------------------------------------

fn session_from_row(row: &PgRow) -> AppResult<Session> {
...
fn session_to_json(session: &Session) -> AppResult<serde_json::Value> {
...
async fn get_session_conn(conn: &mut PgConnection, id: &SessionId) -> AppResult<Option<Session>> {
...
async fn create_session_conn(conn: &mut PgConnection, session: Session) -> AppResult<Session> {
    let data = session_to_json(&session)?;
    match sqlx::query(INSERT_SESSION_SQL)
...
async fn update_session_conn(conn: &mut PgConnection, session: Session) -> AppResult<Session> {
    let existing = sqlx::query(LOCK_SESSION_SQL)
        .bind(session.id.as_str())
        .fetch_optional(&mut *conn)
        .await
        .map_err(db_err)?;
...
async fn revoke_user_sessions_conn(
    ...
    let rows = sqlx::query(LOCK_ACTIVE_SESSIONS_SQL)
        .bind(user_id.as_str())
        .fetch_all(&mut *conn)
        .await
        .map_err(db_err)?;
...
mod tests {
    ...
    fn session_json_round_trips() {
        ...
        let decoded: Session = serde_json::from_value(json).expect("decode");
