//! SQLite friends repository.
//!
//! [`SqliteFriendsRepository`] is the durable single-file backend for
//! [`FriendsRepository`](crate::repository::FriendsRepository). It is the sibling
//! of the Postgres impl in `../pg/friends.rs`: the two directed edges of each
//! relationship are rows in `friend_edges` (`PRIMARY KEY (owner_id, other_id)`)
//! and the invite→mutual / blocked-pair state machine is the shared pure
//! [`plan_add`](crate::repository::friends::plan_add), so the two backends cannot
//! drift.
//!
//! SQLite has no `SELECT ... FOR UPDATE`, so the `add`/`block` read-modify-write
//! runs under `BEGIN IMMEDIATE`, which takes the writer slot up front and
//! serializes the decision the way the Postgres row lock does. `remove` deletes
//! both directed edges in one statement. State is stored as its stable
//! [`FriendState::as_str`] token (parsed back with [`FriendState::from_token`])
...
/// Read one directed edge's state for the read-modify-write in `add` (the
/// `BEGIN IMMEDIATE` writer slot provides the serialization SQLite lacks a
/// `FOR UPDATE` for).
const READ_EDGE_SQL: &str = "SELECT state FROM friend_edges WHERE owner_id = ? AND other_id = ?";
...
fn row_to_friend(row: &SqliteRow) -> AppResult<FriendRow> {
...
/// SQLite [`FriendsRepository`].
pub struct SqliteFriendsRepository {
    executor: SqliteExecutor,
}

impl SqliteFriendsRepository {
...
#[async_trait]
impl FriendsRepository for SqliteFriendsRepository {
...
async fn read_edge_state(
    ...
    let row = sqlx::query(READ_EDGE_SQL)
        .bind(owner)
        .bind(other)
...
async fn remove_conn(conn: &mut SqliteConnection, user: &str, other: &str) -> AppResult<bool> {
...
async fn list_conn(conn: &mut SqliteConnection, user: &str) -> AppResult<Vec<FriendRow>> {
