...
mod python_deadline {
    ...
    use citadel::runtime::{OutboundCommand, PythonRuntime, RpcOutcome, Runtime};

    fn fixture_dir() -> PathBuf {
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/python_deadline")
    }

    #[test]
    fn hung_handlers_return_control_and_leave_runtime_usable() {
        let runtime = PythonRuntime::load(&fixture_dir(), 50)
            .expect("python fixture load succeeds")
            .expect("python deadline fixture is present");
...
#[test]
fn python_runtime_deadline_skips_without_feature() {
    eprintln!("python runtime deadline test skipped: build lacks runtime-python feature");
}
