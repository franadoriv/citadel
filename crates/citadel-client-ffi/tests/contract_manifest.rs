...
use citadel_wire::authoritative_input::{
    CAPABILITY_AUTHORITATIVE_INPUT, CAPABILITY_CHALLENGE_BYTES, CAPABILITY_NEGOTIATION_VERSION,
...
use citadel_wire::protocol::{
    ...
    CHAT_KIND_MIN, KIND_AUTH, KIND_AUTH_RESULT, KIND_AUTHORITATIVE_INPUT,
    KIND_CAPABILITY_ACCEPTANCE, KIND_CAPABILITY_OFFER, KIND_CHAT_EVENT, KIND_DIAG_CAPABILITIES,
    KIND_DIAG_CLOCK_SYNC, KIND_DIAG_FLUSH, KIND_DIAG_SERVER_TIME, KIND_DIAG_START,
    KIND_DIAG_STATUS, KIND_INPUT_STREAM_CONTROL, KIND_MATCH_CLOSED, KIND_MATCHMAKER_MATCHED,
    ...
    KIND_ROOM_MAP_READY, KIND_ROOM_REJECT, KIND_RPC_REQUEST, KIND_RPC_RESPONSE, KIND_TSYNC_ACK,
    KIND_TSYNC_HELLO, KIND_TSYNC_INPUT, KIND_TSYNC_REWIND, KIND_TSYNC_ROLE, KIND_TSYNC_SNAPSHOT,
    KIND_TSYNC_V2_HELLO, KIND_TSYNC_V2_INPUT, KIND_TSYNC_V2_SNAPSHOT, MATCHMAKER_KIND_MAX,
...
use citadel_wire::room::{MATCH_CLOSE_REASON_SERVER_ERROR, MATCH_CLOSED_BYTES};
...
fn render_manifest() -> String {
    ...
    let manifest = serde_json::json!({
        ...
        "wire": {
            ...
            "KIND_TSYNC_V2_HELLO": KIND_TSYNC_V2_HELLO,
            "KIND_TSYNC_V2_SNAPSHOT": KIND_TSYNC_V2_SNAPSHOT,
            "KIND_TSYNC_V2_INPUT": KIND_TSYNC_V2_INPUT,
            ...
            // Room policy rejection (kind 33, outside the pinned 21-25 range;
            // 32 belongs to the match-closed notification, merged first):
            // a policy gate refused a room create/join that used to be silent.
            "KIND_ROOM_REJECT": KIND_ROOM_REJECT,
            "KIND_MATCHMAKER_MATCHED": KIND_MATCHMAKER_MATCHED,
            "KIND_NOTIFICATION": KIND_NOTIFICATION,
            "KIND_CHAT_EVENT": KIND_CHAT_EVENT,
            "KIND_INPUT_STREAM_CONTROL": KIND_INPUT_STREAM_CONTROL,
            "KIND_AUTHORITATIVE_INPUT": KIND_AUTHORITATIVE_INPUT,
            "KIND_CAPABILITY_OFFER": KIND_CAPABILITY_OFFER,
            "KIND_CAPABILITY_ACCEPTANCE": KIND_CAPABILITY_ACCEPTANCE,
            "CAPABILITY_NEGOTIATION_VERSION": CAPABILITY_NEGOTIATION_VERSION,
            "CAPABILITY_AUTHORITATIVE_INPUT": CAPABILITY_AUTHORITATIVE_INPUT,
            "CAPABILITY_CHALLENGE_BYTES": CAPABILITY_CHALLENGE_BYTES,
            "INPUT_STREAM_CONTROL_VERSION": INPUT_STREAM_CONTROL_VERSION,
            "INPUT_STREAM_CONTROL_ADVERTISE": INPUT_STREAM_CONTROL_ADVERTISE,
            "INPUT_STREAM_CONTROL_REVOKE": INPUT_STREAM_CONTROL_REVOKE,
            "INPUT_STREAM_TOKEN_BYTES": INPUT_STREAM_TOKEN_BYTES,
            // Server-driven match closure (S→C, reliable, kind 32): body is
            // room_id + coarse reason + requeue hint; a hinted client
            // re-submits its own matchmaker ticket (the server retains none).
            "KIND_MATCH_CLOSED": KIND_MATCH_CLOSED,
            "KIND_DIAG_SERVER_TIME": KIND_DIAG_SERVER_TIME,
            "KIND_DIAG_CAPABILITIES": KIND_DIAG_CAPABILITIES,
            "KIND_DIAG_CLOCK_SYNC": KIND_DIAG_CLOCK_SYNC,
            "KIND_DIAG_START": KIND_DIAG_START,
            "KIND_DIAG_FLUSH": KIND_DIAG_FLUSH,
            "KIND_DIAG_STATUS": KIND_DIAG_STATUS,
            "MATCH_CLOSE_REASON_SERVER_ERROR": MATCH_CLOSE_REASON_SERVER_ERROR,
            "MATCH_CLOSED_BYTES": MATCH_CLOSED_BYTES,
