...
```js
citadel.log.write(level, tag, message, payloadJson)
```

| Parameter | Type | Required | Meaning |
| --- | --- | --- | --- |
| `level` | string | yes | One of `"trace"`, `"debug"`, `"info"`, `"warn"`, `"error"`; ASCII case is ignored. Unlike `citadel.log`, an unrecognized level **throws**, never falls back to `info` — a stored row that says `info` when the author wrote `eror` is a lie no operator can detect later. |
...
| `payloadJson` | string | no | A JSON **object or array**, at most 8192 bytes (`logs.match_logs.max_payload_bytes`). Pass `null` or omit it for none. Stored verbatim and never inspected. |
...
**Errors:** throws when the level, tag, message, or payload fails the rules
above, and throws `durable logs are unavailable` on a node with no durable log
tables.

```js
citadel.log.write("info", "combat.hit", "kitsune scored");
citadel.log.write("warn", "economy", "refund issued", JSON.stringify({ amount: 250 }));
```
...
```js
citadel.match.set_result(resultJson)
```

| Parameter | Type | Required | Meaning |
| --- | --- | --- | --- |
| `resultJson` | string | yes | A JSON **object**, at most 4096 bytes. Stored verbatim on the match record. |
...
**Errors:** throws `match results require a match-scoped context` outside a
match-scoped callback, `match result exceeds the maximum size` past 4096 bytes,
a message naming the shape for a document that is not a JSON object, or
`durable logs are unavailable` on a node with no durable log tables.

```js
citadel.on_match_ended((ctx) => {
  citadel.match.set_result(JSON.stringify({ winner: "kitsune", rounds: 3 }));
});
```
