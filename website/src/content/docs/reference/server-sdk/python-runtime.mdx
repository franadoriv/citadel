...
returns an opaque runtime-local `int` handle immediately; it never waits for
network I/O. `url` must be an `http` or `https` DNS-hostname URL. `opts` may
contain `method` (string, default `"GET"`), `headers` (a `dict[str, str]`), and
`body` (`bytes` or `str`). Rust policy rejections raise their stable code without
allocating a handle. Local Python argument or option validation can instead
raise a Python-visible validation exception; it is not an `error_code` contract.
...
or reload-invalidated handles raise an error. Reload and shutdown cancel and
forget all handles. `fetch`, `start`, `poll`, and `cancel` all raise
`RuntimeError("interceptor_forbidden")` inside realtime interceptors.
