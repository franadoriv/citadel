...
| `adapter` | enum | `"embedded"` | Runtime hosting adapter. `"embedded"` (the default) executes game scripts in-process. `"external-worker"` executes them in a supervised worker process instead: each live match runs in its own engine context (own Lua/JS state, own Python module namespace) with fair scheduling and crash containment — a wedged or crashed script costs the worker process, never the server; affected matches close with a requeue hint. Match-independent surfaces (global messages, RPC, lifecycle hooks, custom HTTP endpoints) are not routed to the worker yet. `"wasm"` is reserved and currently fails validation. |
