...
2. A **realtime delivery pattern** built from primitives that already ship:
   ...
   kind. There is no chat-specific wire kind or client method yet — see the
   note in [Realtime delivery today](#realtime-delivery-today).
...
There is **no dedicated chat wire kind and no chat-specific client SDK
method** yet. What exists today is the same general-purpose relay pattern
...
or `citadel.send` (one session). Reserved kinds run through 26 (see the
...
must be **27 or higher**.

:::note[Future work — not implemented]
A dedicated chat wire protocol and chat-specific client SDK API (message
history sync, per-channel subscribe/unsubscribe, typing indicators, etc.) are
**future work**. The
pattern below is the *only* way chat messages reach a connected game client
today: a plain custom envelope kind, relayed by your own Lua script. It is
not chat-aware — the server does not know `kind 100` means "chat message" —
your game defines that convention.
:::
...
- **No chat-specific client SDK surface.** The relay pattern above uses only
  generic envelope send/receive; there is no `SendChatMessage`/`OnChatMessage`
  style method on any client SDK today.
