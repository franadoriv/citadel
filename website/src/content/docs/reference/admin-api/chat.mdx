...
`website/src/content/docs/reference/client-sdk/godot-web.mdx`). The calls above are the intended
surface and will work once the native extension lands; today `send`/`poll`
return an error status instead of moving bytes.
:::
...
- **Blueprint cannot send/receive raw envelope kinds today.** See the
  Blueprint tab above.
- **Godot's native client is a skeleton.** `send`/`poll` do not move bytes
  until the GDExtension is wired up.
