...
that native binary, the calls return an error status through `last_error` (the
SDK's "skeleton" fallback) instead of moving bytes. For a browser export, use
`CitadelWebClient`, which needs no GDExtension — see the
